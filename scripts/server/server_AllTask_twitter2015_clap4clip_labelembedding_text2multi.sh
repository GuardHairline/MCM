...
# SERVER - ALL TASKS - TWITTER2015 - CLAP4CLIP_LABELEMBEDDING - TEXT2MULTI
# CLAP4CLIP + Label Embedding - 先执行text_only，再执行multimodal
...
# 标签嵌入配置
LABEL_EMB_PATH="$CHECKPOINT_DIR/label_embedding_twitter2015.pt"

echo "=== Starting text2multi training on TWITTER2015 with CLAP4CLIP_LABELEMBEDDING ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/twitter2015_clap4clip_labelembedding_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_twitter2015_clap4clip_labelembedding_text"
    
    # 根据任务确定实际数据集
    if [ "$TASK_NAME" = "mner" ]; then
        ACTUAL_DATASET="twitter2015_ner"
        DATASET_NAME="twitter2015"
        DATA_DIR="./data"
    else
        ACTUAL_DATASET="twitter2015"
        DATASET_NAME="twitter2015"
        DATA_DIR="./data"
    fi
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2015_clap4clip_labelembedding_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2015_clap4clip_labelembedding_text.log"
    
    # 获取数据集文件路径
    if [ "$TASK_NAME" = "mner" ]; then
        TRAIN_FILE="data/MNER/twitter2015/train.txt"
        DEV_FILE="data/MNER/twitter2015/dev.txt"
        TEST_FILE="data/MNER/twitter2015/test.txt"
        if [ "twitter2015" = "200" ]; then
        ...
        TRAIN_FILE="data/MASC/twitter2015/train.txt"
        DEV_FILE="data/MASC/twitter2015/dev.txt"
        TEST_FILE="data/MASC/twitter2015/test.txt"
        if [ "twitter2015" = "200" ]; then
    ...
    echo "Task: $TASK_NAME, Dataset: twitter2015 (actual: $ACTUAL_DATASET)"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_twitter2015_clap4clip_labelembedding_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2015_clap4clip_labelembedding_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2015_clap4clip_labelembedding_multi.log"
