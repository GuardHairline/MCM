...
# SERVER - ALL TASKS - MIX - LABELEMBEDDING - MULTIMODAL
# Label Embedding
...
echo "=== Starting multi-task training on MIX with LABELEMBEDDING ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/mix_labelembedding_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_mix_labelembedding"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_labelembedding_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_labelembedding_multimodal.log"
