...
# SERVER - ALL TASKS - MIX - SI - MULTIMODAL
...
echo "=== Starting multi-task training on MIX with SI ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/mix_si_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_mix_si"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_si_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_si_multimodal.log"
