...
# SERVER - ALL TASKS - TWITTER2017 - MAS - TEXT2MULTI
...
echo "=== Starting text2multi training on TWITTER2017 with MAS ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/twitter2017_mas_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_twitter2017_mas_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2017_mas_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2017_mas_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_twitter2017_mas_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2017_mas_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2017_mas_multi.log"
