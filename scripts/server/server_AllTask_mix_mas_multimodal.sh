...
# SERVER - ALL TASKS - MIX - MAS - MULTIMODAL
...
echo "=== Starting multi-task training on MIX with MAS ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/mix_mas_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_mix_mas"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_mas_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_mas_multimodal.log"
