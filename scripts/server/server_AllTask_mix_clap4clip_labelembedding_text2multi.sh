...
for i in "${!TASKS[@]}"; do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_${i+1}_$TASK_NAME_mix_clap4clip_labelembedding"
    ...
    if [ "server" = "server" ]; then
        ...
        MODEL_PATH="$CHECKPOINT_DIR/$TASK_NAME_mix_clap4clip_labelembedding.pt"
    fi
    
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_clap4clip_labelembedding.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_clap4clip_labelembedding.log"
