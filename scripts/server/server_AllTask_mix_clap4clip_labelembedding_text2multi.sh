...
# SERVER - ALL TASKS - MIX - CLAP4CLIP_LABELEMBEDDING - TEXT2MULTI
...
# 标签嵌入配置
LABEL_EMB_PATH="$CHECKPOINT_DIR/label_embedding_mix.pt"

echo "=== Starting text2multi training on MIX with CLAP4CLIP_LABELEMBEDDING ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/mix_clap4clip_labelembedding_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_mix_clap4clip_labelembedding_text"
    
    # 根据任务确定实际数据集
    if [ "$TASK_NAME" = "mner" ]; then
        ACTUAL_DATASET="mix_ner"
        DATASET_NAME="mix"
        DATA_DIR="./data"
    else
        ACTUAL_DATASET="mix"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_clap4clip_labelembedding_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_clap4clip_labelembedding_text.log"
    
    # 获取数据集文件路径
    if [ "$TASK_NAME" = "mner" ]; then
        TRAIN_FILE="data/MNER/mix/train.txt"
        DEV_FILE="data/MNER/mix/dev.txt"
        TEST_FILE="data/MNER/mix/test.txt"
        if [ "mix" = "200" ]; then
        ...
        TRAIN_FILE="data/MASC/mix/train.txt"
        DEV_FILE="data/MASC/mix/dev.txt"
        TEST_FILE="data/MASC/mix/test.txt"
        if [ "mix" = "200" ]; then
    ...
    echo "Task: $TASK_NAME, Dataset: mix (actual: $ACTUAL_DATASET)"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_mix_clap4clip_labelembedding_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_clap4clip_labelembedding_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_clap4clip_labelembedding_multi.log"
