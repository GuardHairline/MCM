...
for i in "${!TASKS[@]}"; do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_${i+1}_$TASK_NAME_mix_moe"
    ...
    if [ "server" = "server" ]; then
        ...
        MODEL_PATH="$CHECKPOINT_DIR/$TASK_NAME_mix_moe.pt"
    fi
    
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_moe.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_moe.log"
