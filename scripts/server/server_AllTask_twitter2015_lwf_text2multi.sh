...
for i in "${!TASKS[@]}"; do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_${i+1}_$TASK_NAME_twitter2015_lwf"
    ...
    if [ "server" = "server" ]; then
        ...
        MODEL_PATH="$CHECKPOINT_DIR/$TASK_NAME_twitter2015_lwf.pt"
    fi
    
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2015_lwf.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2015_lwf.log"
