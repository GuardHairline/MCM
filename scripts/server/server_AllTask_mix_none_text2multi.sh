...
# SERVER - ALL TASKS - MIX - NONE - TEXT2MULTI
...
echo "=== Starting text2multi training on MIX with NONE ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/mix_none_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_mix_none_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_none_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_none_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_mix_none_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_mix_none_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_mix_none_multi.log"
