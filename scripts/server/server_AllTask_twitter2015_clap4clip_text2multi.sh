...
# SERVER - ALL TASKS - TWITTER2015 - CLAP4CLIP - TEXT2MULTI
...
echo "=== Starting text2multi training on TWITTER2015 with CLAP4CLIP ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/twitter2015_clap4clip_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_twitter2015_clap4clip_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2015_clap4clip_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2015_clap4clip_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_twitter2015_clap4clip_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2015_clap4clip_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2015_clap4clip_multi.log"
