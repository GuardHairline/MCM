...
# SERVER - ALL TASKS - TWITTER2017 - EWC - TEXT2MULTI
...
echo "=== Starting text2multi training on TWITTER2017 with EWC ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/twitter2017_ewc_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_twitter2017_ewc_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2017_ewc_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2017_ewc_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_twitter2017_ewc_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2017_ewc_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2017_ewc_multi.log"
