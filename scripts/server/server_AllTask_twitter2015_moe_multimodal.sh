...
# SERVER - ALL TASKS - TWITTER2015 - MOE - MULTIMODAL
# MoE Adapters
...
echo "=== Starting multi-task training on TWITTER2015 with MOE ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/twitter2015_moe_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_twitter2015_moe"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2015_moe_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2015_moe_multimodal.log"
