...
# SERVER - ALL TASKS - TWITTER2017 - EWC - MULTIMODAL
...
echo "=== Starting multi-task training on TWITTER2017 with EWC ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/twitter2017_ewc_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_twitter2017_ewc"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_twitter2017_ewc_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_twitter2017_ewc_multimodal.log"
