#!/usr/bin/env python3
"""
Kaggle运行脚本 - Account 3 - MABSA (Baseline vs CRF)
...
print("="*80)
print("Account 3 - MABSA (Baseline vs CRF)")
print("="*80)
print(f"任务: MABSA")
print(f"配置数: 2")
print(f"预计时间: 440 分钟")
...
configs = [
  {
    "file": "kaggle_baseline_twitter2015_mabsa.json",
    "ablation": "baseline"
  },
  {
    "file": "kaggle_crf_only_twitter2015_mabsa.json",
    "ablation": "crf_only"
  }
]
...
for i, config_info in enumerate(configs, 1):
    config_file = Path("scripts/configs/kaggle_ablation/account_3") / config_info["file"]
    ...
    # 保存中间结果
    with open(Path(KAGGLE_WORKING) / "account_3_results.json", 'w') as f:
...
final_results = {
    ...
    "account_name": "Account 3 - MABSA (Baseline vs CRF)",
    "task": "mabsa",
...
output_file = Path(KAGGLE_WORKING) / "account_3_final_results.json"
...
# 打包1: metrics和摘要JSON
zip_path = Path(KAGGLE_WORKING) / "account_3_results.zip"
...
# 打包2: 单独压缩所有模型文件（.pt）
print("\n压缩模型文件...")
models_zip_path = Path(KAGGLE_WORKING) / "account_3_models.zip"
pt_files_count = 0
