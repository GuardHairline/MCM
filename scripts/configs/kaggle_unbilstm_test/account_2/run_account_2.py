#!/usr/bin/env python3
"""
Kaggle运行脚本 - Account 2 - MNER BiLSTM测试
...
def run_experiments(project_dir: Path):
    ...
    configs = [{"file": "kaggle_bilstm_config_default_twitter2015_mner.json", "task": "mner", "config_type": "config_default"}]
    
    results = {}
    
    for idx, config_info in enumerate(configs, 1):
        config_file = config_info["file"]
        config_path = project_dir / "scripts" / "configs" / "kaggle_bilstm_test" / "account_2" / config_file
...
def collect_results(project_dir: Path, results: dict):
    ...
    # 创建结果目录
    results_dir = Path(KAGGLE_WORKING) / "results_account_2"
    ...
    # 保存运行摘要
    summary_file = results_dir / "run_summary_account_2.json"
