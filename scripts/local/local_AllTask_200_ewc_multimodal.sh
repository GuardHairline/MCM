...
# LOCAL - ALL TASKS - 200 - EWC - MULTIMODAL
...
echo "=== Starting multi-task training on 200 with EWC ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/200_ewc_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_200_ewc"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_ewc_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_ewc_multimodal.log"
