...
# LOCAL - ALL TASKS - 200 - CLAP4CLIP - MULTIMODAL
# CLAP4CLIP
...
echo "=== Starting multi-task training on 200 with CLAP4CLIP ==="
...
PREV_MODEL=""
FINAL_MODEL="$CHECKPOINT_DIR/200_clap4clip_multimodal.pt"
TMP_MODEL="$CHECKPOINT_DIR/1.pt"

# 逐个训练任务
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_200_clap4clip"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_clap4clip_multimodal.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_clap4clip_multimodal.log"
