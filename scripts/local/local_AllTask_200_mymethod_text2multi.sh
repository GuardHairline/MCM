...
for i in "${!TASKS[@]}"; do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_${i+1}_$TASK_NAME_200_mymethod"
    ...
    if [ "local" = "server" ]; then
        ...
        MODEL_PATH="$CHECKPOINT_DIR/$TASK_NAME_200_mymethod.pt"
    fi
    
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_mymethod.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_mymethod.log"
