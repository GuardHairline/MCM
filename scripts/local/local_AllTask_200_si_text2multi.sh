...
# LOCAL - ALL TASKS - 200 - SI - TEXT2MULTI
...
echo "=== Starting text2multi training on 200 with SI ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/200_si_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_200_si_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_si_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_si_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_200_si_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_si_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_si_multi.log"
