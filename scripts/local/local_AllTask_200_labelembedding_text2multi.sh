...
# LOCAL - ALL TASKS - 200 - LABELEMBEDDING - TEXT2MULTI
...
echo "=== Starting text2multi training on 200 with LABELEMBEDDING ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/200_labelembedding_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_200_labelembedding_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_labelembedding_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_labelembedding_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_200_labelembedding_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_labelembedding_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_labelembedding_multi.log"
