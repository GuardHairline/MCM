...
# LOCAL - ALL TASKS - 200 - MAS - TEXT2MULTI
...
echo "=== Starting text2multi training on 200 with MAS ==="
...
FINAL_MODEL="$CHECKPOINT_DIR/200_mas_t2m.pt"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+1))_${TASK_NAME}_200_mas_text"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_mas_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_mas_text.log"
...
for (( ; i<${#TASKS[@]}; i++ )); do
    TASK_NAME="${TASKS[$i]}"
    SESSION_NAME="session_$((i+5))_${TASK_NAME}_200_mas_multi"
    ...
    TRAIN_INFO_PATH="$CHECKPOINT_DIR/train_info_200_mas_t2m.json"
    LOG_FILE="$LOG_DIR/$TASK_NAME_200_mas_multi.log"
