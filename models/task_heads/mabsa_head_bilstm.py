...
标签: O=0, B-POS, I-POS, B-NEG, I-NEG, B-NEU, I-NEU (7个标签)
...
class MABSAHeadBiLSTM(nn.Module):
    """
    MABSA (Multimodal Aspect-Based Sentiment Analysis) 任务头 - BiLSTM增强版
    
    标签: O=0, B-POS=1, I-POS=2, B-NEG=3, I-NEG=4, B-NEU=5, I-NEU=6 (7个标签)
    
    参数:
        input_dim: 输入特征维度（默认768）
        num_labels: 标签数量（MABSA固定为7）
    ...
    def __init__(
        self,
        input_dim: int = 768,
        num_labels: int = 7,  # O, B-POS, I-POS, B-NEG, I-NEG, B-NEU, I-NEU
        hidden_size: int = 256,
        ...
        if enable_bilstm:
            ...
            print("⚠️ MABSAHeadBiLSTM: BiLSTM disabled, using direct projection.")
        ...
        if self.use_crf:
            ...
            print(f"✓ MABSAHeadBiLSTM initialized with CRF ({'torchcrf' if HAS_TORCHCRF else 'SimpleCRF'})")
        else:
            self.crf = None
            print("✓ MABSAHeadBiLSTM initialized without CRF")
...
def test_mabsa_head_bilstm():
    """测试MABSAHeadBiLSTM"""
    print("="*80)
    print("测试 MABSAHeadBiLSTM")
    print("="*80)
    ...
    seq_len = 32
    input_dim = 768
    num_labels = 7  # O, B-POS, I-POS, B-NEG, I-NEG, B-NEU, I-NEU
    
    # 创建模型
    print("\n1. 创建模型（使用CRF）...")
    head_with_crf = MABSAHeadBiLSTM(
        input_dim=input_dim,
